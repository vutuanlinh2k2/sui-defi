...
use prize_savings::test_utils::{deposit_coin_to_address, set_timestamp_s};
...
public fun test_pool() {
    ...
    test.next_tx(ALICE);
    set_timestamp_s(&test, 10_000);
    test.next_tx(ALICE);
    deposit(&mut test);

    // * Bob deposit 1_500_000_000 SUI in 15000 sec
    test.next_tx(BOB);
    set_timestamp_s(&test, 15_000);
    test.next_tx(BOB);
    deposit(&mut test);
    ...
    test.next_tx(BOB);
    set_timestamp_s(&test, 25_000);
    test.next_tx(BOB);
    deposit(&mut test);

    // * Alice withdraw 1_000_000_000 PToken in 75000 sec
    test.next_tx(ALICE);
    set_timestamp_s(&test, 75_000);
    test.next_tx(ALICE);
    {
    ...
    {   
        ...
        assert!(prize_pool.prize_balances_amount() == 499_999_998); // not 500_000_000 due to floor
        let twab_controller_snapshot = prize_pool.twab_controller_snapshot();
        let twab_total = twab_controller_snapshot.get_twab_total(
            prize_pool.start_timestamp_s(), 
            prize_pool.end_timestamp_s()
        );
        let twab_alice = twab_controller_snapshot.get_twab_by_user(
            ALICE, 
            prize_pool.start_timestamp_s(), 
            prize_pool.end_timestamp_s()
        );
        let twab_bob = twab_controller_snapshot.get_twab_by_user(
            BOB, 
            prize_pool.start_timestamp_s(), 
            prize_pool.end_timestamp_s()
        );

        assert!(twab_total == 3_820_987_653);
        assert!(twab_alice == 2_047_067_900);
        assert!(twab_bob == 1_773_919_752);

        return_shared(pool);
        return_shared(prize_pool);
        destroy(twab_controller_snapshot);  
    };
