...
public(package) fun create_prize_pool<T>(
  pool_id: ID,
  draw_id: u64,
  prize_count_per_tier: vector<u8>,
  prize_per_winner_per_tier: vector<u64>,
  ...
  let prize_claimed_per_tier = vector::empty<u8>();
...
fun calculate_pseudo_random_number(
    ...
    vector::append(&mut bytes, bcs::to_bytes(&winning_random_number));
